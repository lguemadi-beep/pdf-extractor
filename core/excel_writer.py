...
def _facture_to_rows(fac: Facture) -> list[tuple]:
    ...
    (Status defaults to "Nouveau") for manual follow-up in Excel; existing
    values already typed in for older rows are preserved on later runs.
    """
    if fac.lignes:
        return [
            (fac.societe, fac.demandeur, fac.numero, fac.date, ligne.designation, ligne.montant_ht, "", "", "Nouveau")
            for ligne in fac.lignes
        ]
    # Invoice with no parsed line items: still show one row so it isn't lost
    return [(fac.societe, fac.demandeur, fac.numero, fac.date, "", fac.sous_total_ht, "", "", "Nouveau")]
...
def _fill_invoice_summary_sheet(ws: Worksheet, invoice_rows: list[tuple], source_folder: str) -> Worksheet:
    ...
    headers = ["Fournisseur", "Demandeur", "Numéro facture", "Date de facture", "Montant HT", "Owner", "Affectation", "Status"]
    _write_header(ws, headers, row=header_row)
    r = header_row + 1
