...
def _make_columns_readonly(ws: Worksheet, columns: list[int], first_row: int, last_row: int = 1000) -> None:
    """
    Blocks manual edits to the given columns WITHOUT using Excel sheet
    protection — confirmed by direct testing that turning on sheet
    protection (ws.protection.sheet = True) makes the Owner/Status
    dropdown arrows stop appearing in at least one real-world Excel
    install, which matters more than strict enforcement. Instead, each
    cell gets a "reject everything" custom data validation rule: typing
    any new value shows an error and the edit is rejected, while the
    existing extracted value stays exactly as written by the app. This
    coexists cleanly with the separate Owner/Status list validations
    since data validation doesn't depend on sheet protection at all.
    """
    for col in columns:
        ...
        dv.error = "Cette colonne est générée automatiquement et n'est pas modifiable."
        dv.errorTitle = "Colonne en lecture seule"
        dv.add(f"{col_letter}{first_row}:{col_letter}{last_row}")
        ws.add_data_validation(dv)
...
def _protect_extracted_columns(
    ...
    """
    Marks the extracted-data columns as locked and the manual tracking
    columns (Owner CPM / BA, Affectation, Status) as unlocked — but does
    NOT turn on sheet protection itself. Confirmed by direct testing:
    enabling sheet protection (even with these same cells correctly
    unlocked) makes the Owner/Status dropdown arrows stop appearing in at
    least one real-world Excel install, which matters more than strict
    read-only enforcement. If read-only enforcement is wanted, it can be
    turned on manually in Excel via Review > Protect Sheet — the correct
    locked/unlocked state per column is already baked in, so that will
    "just work" without needing to redo anything.
    """
...
def _autosize(
    ...
    max_width: int = 60,
    start_row: int = 1,
    column_min_overrides: dict[int, int] | None = None,
    column_max_overrides: dict[int, int] | None = None,
) -> None:
    """
    ...
    column_min_overrides lets specific columns (by 1-based index) use a
    higher minimum — used for the Owner/Status dropdown columns so they're
    wide enough for the longest option even before anyone picks a value.
    column_max_overrides lets specific columns use a higher (or lower)
    cap than the shared max_width — used for the Description column so
    it follows its real (often long) content instead of being capped at
    the same width as every other column.
    """
    column_min_overrides = column_min_overrides or {}
    column_max_overrides = column_max_overrides or {}
    for col_cells in ws.columns:
        ...
        col_min = max(min_width, column_min_overrides.get(col_index, 0))
        col_max = column_max_overrides.get(col_index, max_width)
        ws.column_dimensions[col_letter].width = max(col_min, min(col_max, length + 4))
...
def _fill_factures_sheet(ws: Worksheet, rows: list[tuple], source_folder: str, invoice_count: int) -> Worksheet:
    ...
    _autosize(
        ws, start_row=header_row,
        column_min_overrides={
            7: max(len(o) for o in OWNER_OPTIONS) + 2,
            9: max(len(o) for o in STATUS_OPTIONS) + 2,
        },
        column_max_overrides={5: 110},
    )
    # Applied LAST: fill/border operations above (_band_rows, TOTAL row) would
    # otherwise silently reset the per-cell locked/unlocked protection flag
    # set here, which is what made the Owner/Status dropdowns stop showing
    # once sheet protection was turned on.
    if last_data_row >= header_row + 1:
        ...
        _make_columns_readonly(ws, columns=[1, 2, 3, 4, 5, 6], first_row=header_row + 1, last_row=last_data_row)
    return ws
...
def _fill_invoice_summary_sheet(ws: Worksheet, invoice_rows: list[tuple], source_folder: str) -> Worksheet:
    ...
    _autosize(
        ws, start_row=header_row,
        column_min_overrides={
            6: max(len(o) for o in OWNER_OPTIONS) + 2,
            8: max(len(o) for o in STATUS_OPTIONS) + 2,
        },
    )
    if last_data_row >= header_row + 1:
        _protect_extracted_columns(
        ...
        _make_columns_readonly(ws, columns=[1, 2, 3, 4, 5], first_row=header_row + 1, last_row=last_data_row)
    return ws
